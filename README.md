...
- **Combine online sheets** — a button that lets you multi-select several Google Sheets from Drive at once. After picking them, you're asked one simple question: put them together normally (stack every sheet's rows into one table — the default, same as before), or link them by a relationship instead (pick matching column(s) between the first sheet and each of the others, like a VLOOKUP — repeated rows on either side get totaled automatically first, so this stays correct even when a sheet has several rows per matching key) — entirely optional, right there in the same step, no separate panel to dig through.
...
- **Merge (join)** — combines a main table with one or more others by matching column(s), like a VLOOKUP/left join done any number of times. Two ways to feed it, picked with a small switch at the top of this mode:
  ...
  **Only the columns you actually pick make it into the merged result.** Under the base table and every linked sheet, a checklist lets you pick exactly which of that sheet's other columns to bring in — each with its own combine method (Sum, Average, Count, Max, Min, Distinct count, or First value — the last one for an ID/name/category column that shouldn't be combined at all). Unchecked columns are left out entirely, not just un-aggregated — this matters a lot for a raw export with a lot of columns nobody asked for (a Stock-move/Odoo-style export easily has 15+ columns, most only populated on some rows): picking just the few columns actually needed keeps the merged result small and exact instead of dragging every column along. Numeric-looking columns are pre-checked with Sum by default; everything else starts unchecked — uncheck/check and change the combine method for any column freely before applying.

  **After applying, a summary tells you if anything didn't match** — e.g. "12 of 22,840 base rows found no match; 5 of 340 Scrap entries were never used" — instead of leaving you to notice a suspicious value (or a suspiciously plain 0) later. A row failing to match despite the key genuinely being the same on both sides is almost always a formatting difference the key comparison doesn't yet normalize for (dates and plain numbers already are — see above); if you still see unexpected unmatched counts, double-check for things like extra whitespace, or an ID stored as text on one side and a number on the other.

  **A linked sheet's entries with no matching base row are dropped by default** (a normal left join — only base rows survive). Check **"Also keep entries that don't have a matching row in the base table"** under any linked sheet to keep those too, as extra rows with every base-only column left blank — useful when a lookup sheet's own entries matter even without a matching base row, e.g. wanting every Scrap entry to show up somewhere even for a date/item no Sales row was ever recorded for.
